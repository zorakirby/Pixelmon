...
public class BlockFossilMachine extends BlockContainer {
    ...
    {
        return new ItemStack(PixelmonItems.fossilMachineItem, 1);
    }
	...
	public void randomDisplayTick(World par1World, int par2, int par3, int par4, Random par5Random)
    {
        ...
        if(tile.fossilProgress > 0 || tile.pokemonProgress > 0)
        	for(int i = 50; i > ((int)(((tile.fossilProgress + tile.pokemonProgress) * 2) / 96) / 2); i--)
        		par1World.spawnParticle("reddust", var7 + par5Random.nextFloat(), var9 + par5Random.nextFloat() + par5Random.nextFloat(), var11 + par5Random.nextFloat(), -255D, 1.0D, 255.0D);
...
//		Item Placing
		...
		if (player.getCurrentEquippedItem() != null && (player.getCurrentEquippedItem().getItem() instanceof ItemPokemonFossil) && tile.currentFossil == -1 && !tile.pokemonOccupied) {
...
//		Pokemon retrieval
		...
		if (!tile.pokemonOccupied && player.getCurrentEquippedItem() != null && !(player.getCurrentEquippedItem().getItem() instanceof ItemPokemonFossil && !(player.getCurrentEquippedItem().getItem() instanceof ItemPokeBall))) {
			tile.pokemonOccupied = false;
			tile.fossilProgress = 0.0f;
			tile.pokemonProgress = 0.0f;
			tile.currentFossil = -1;
	...
	public int idDropped(int par1, Random par2Random, int par3) {
		return PixelmonItems.fossilMachineItem.shiftedIndex;
	}
