...
public class Pokedex {
	...
	private ArrayList<Object[]> pokedex = new ArrayList<Object[]>();
	...
	public Pokedex() {
		...
		try {
			while (r.next()) {
				...
				p.rarity = rarity;
				p.name = s;
				// PokedexEntry RareTest = new PokedexEntry(i+1, s,
				// r.getString("Rarity")).setHeightAndWeight(d, d1);
				pokedexMap.put(s, p);
				// pokedexMap.put(s, RareTest);
				pokedex.add(new Object[] { i, p });
				pokedexList.add(p);
	...
	public PokedexEntry getEntry(int i) {
		...
		for (Object[] o : pokedex) {
			PokedexEntry e = (PokedexEntry) o[1];
			int i1 = ((Number) o[0]).intValue();
			if (i1 == i) {
				e1 = e;
				break;
			}
