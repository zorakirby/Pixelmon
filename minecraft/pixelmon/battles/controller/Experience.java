...
/**
 * Experience: Methods for awarding EXP to a team's pokemon.
 * Class made final since it has no instance variables/methods.
 */
public final class Experience
{

   /**
    * Award EXP to the pokemon of battle participants after an opposing pokemon faints.
    * @param participants list of team owners to award EXP to
    * @param losingTeamOwner owner of faintedPokemon, will be a player, an NPC, or wild (no owner)
    * @param faintedPokemon defeated pokemon to base EXP reward on
    */
   public static void awardExp(ArrayList<BattleParticipant> participants,
      BattleParticipant losingTeamOwner, EntityPixelmon faintedPokemon)
   {
      if (!PixelmonConfig.allowPVPExperience)
      {
         ...
         if (allPlayers)
         {
            // Award no EXP to anyone if all participants are players with PVP EXP gain configured off.
            return;
         }
      }
      try
      {
         for (BattleParticipant teamOwner : participants)
         {
            ...
            {
               // Begin EXP-awarding logic for a team owner if it is a player and if
               ...
               PlayerStorage storage = PixelmonStorage.PokeballManager.getPlayerStorage(
                  (EntityPlayerMP) activePokemon.getOwner());
                     ...
                     if (nbt.hasKey("HeldItem"))
                     {
                        ...
                        if (itemId != -1)
                        {
                           if (PixelmonItemsHeld.getHeldItem(itemId).getHeldItemType() == EnumHeldItems.expShare
                              && !nbt.getBoolean("IsFainted") && nbt.getShort("Health") > 0)
                           {
                              calcExp(storage, activePokemon, faintedPokemon, nbt.getInteger("pixelmonID"),
                                 0.5 / (double) expShareCount);
                           }
                        }
                     }
                  }
               ...
               for (int userIndex : enemyIdList)
               {
                  ...
                  {
                     calcExp(storage, activePokemon, faintedPokemon, userIndex,
                        expShareCount > 0 ? 0.5 : 1);
   ...
   /**
    * Calculate EXP and EV gains of a pokemon from base stats of the pokemon and
    * its fainted opponent.
    * Note: isSharedExp was unused.
    * @param storage player pokemon belt storage
    * @param battlingPokemon the sent-out, battling pokemon of the player
    * @param faintedPokemon defeated pokemon to base EXP reward on
    * @param expReceiverIndex integer index of the EXP-receiving pokemon; will
    * be the battling pokemon's in one case
    * @param scaleFactor total EXP multiplier
    */
   private static void calcExp(PlayerStorage storage, EntityPixelmon battlingPokemon,
      EntityPixelmon faintedPokemon, int expReceiverIndex, double scaleFactor)
   {
      ...
      // Note: Multiplier has always been applied to party pokemon as long as the battling pokemon is holding it.
      ...
      double exp = ((a * baseExp * faintedLevel) / (5 * s) * (Math.pow(2 * faintedLevel + 10, 2.5) / Math.pow(faintedLevel + Lp + 10, 2.5)) + 1) * t * eggMultiplier * power;
      exp = exp * scaleFactor;
      if (expReceiverIndex == battlingPokemon.getPokemonId())
      {
         ...
         {
            battlingPokemon.stats.EVs.gainEV(faintedPokemon.baseStats.evGain);
         }
      ...
      else
      {
         ...
         {
            // Sending out stored pokemon so player may see it for potential levelling up.
