...
public class RenderTileFossilMachine extends TileEntitySpecialRenderer {
	...
	private ModelOldAmber oldAmber;
	private ModelDomeFossil domeFossil;
	private ModelRootFossil rootFossil;
	private ModelSkullFossil skullFossil;
	private ModelClawFossil clawFossil;
	private ModelHelixFossil helixFossil;
	private ModelCoverFossil coverFossil;
	private EntityPixelmon pokemon;
	
	public RenderTileFossilMachine() {
		model = new ModelFossilMachine();
		pokeball = new ModelPokeball();
		oldAmber = new ModelOldAmber();
		domeFossil = new ModelDomeFossil();
		rootFossil = new ModelRootFossil();
		skullFossil = new ModelSkullFossil();
		clawFossil = new ModelClawFossil();
		helixFossil = new ModelHelixFossil();
		coverFossil = new ModelCoverFossil();
	}
	...
	public String fossilTexture(TileEntityFossilMachine tile){
		if(tile.currentFossil == PixelmonItems.oldAmber.shiftedIndex){
			return "oldamber";
		}
		if(tile.currentFossil == PixelmonItems.domeFossil.shiftedIndex){
			return "domefossil";
		}
		if(tile.currentFossil == PixelmonItems.helixFossil.shiftedIndex){
			return "helixfossil";
		}
		if(tile.currentFossil == PixelmonItems.armorFossil.shiftedIndex){
			return "armorfossil";
		}
		if(tile.currentFossil == PixelmonItems.rootFossil.shiftedIndex){
			return "rootfossil";
		}
		if(tile.currentFossil == PixelmonItems.clawFossil.shiftedIndex){
			return "clawfossil";
		}
		if(tile.currentFossil == PixelmonItems.skullFossil.shiftedIndex){
			return "skullfossil";
		}
		if(tile.currentFossil == PixelmonItems.coverFossil.shiftedIndex){
			return "coverfossil";
		}
		if(tile.currentFossil == PixelmonItems.plumeFossil.shiftedIndex){
			return "plumefossil";
		}
		else return "";
	}
	public void renderModel(TileEntityFossilMachine tile, float f){
		if(tile.currentFossil == PixelmonItems.oldAmber.shiftedIndex){
			oldAmber.renderModel(tile, f);
		}
		if(tile.currentFossil == PixelmonItems.domeFossil.shiftedIndex){
			domeFossil.renderModel(tile, f);
		}
		if(tile.currentFossil == PixelmonItems.rootFossil.shiftedIndex){
			rootFossil.renderModel(tile, f);
		}
		if(tile.currentFossil == PixelmonItems.skullFossil.shiftedIndex){
			skullFossil.renderModel(tile, f);
		}
		if(tile.currentFossil == PixelmonItems.clawFossil.shiftedIndex){
			clawFossil.renderModel(tile, f);
		}
		if(tile.currentFossil == PixelmonItems.helixFossil.shiftedIndex){
			helixFossil.renderModel(tile, f);
		}
		if(tile.currentFossil == PixelmonItems.coverFossil.shiftedIndex){
			coverFossil.renderModel(tile, f);
		}
	}

	@Override
	public void renderTileEntityAt(TileEntity tileentity, double d, double d1, double d2, float f) {
		renderAModelAt((TileEntityFossilMachine) tileentity, d, d1, d2, f); // where to
																	// render
	}
}
