...
public class SpawnRegistry {
	...
	public static void addPixelmonSpawn(String name, BaseStats stats, ClassType type) {
		...
		if (stats.spawnLocations == null) {
			for (BiomeGenBase b : biomes) {
				storeSpawnInfo(biomeSpawns, name, stats.rarity, type, b, null);
			}
		} else {
				...
				for (BiomeGenBase b : biomes) {
					if (type == ClassType.Pixelmon) {
						if (s == SpawnLocation.Land || s == SpawnLocation.Air) {
							storeSpawnInfo(biomeSpawns, name, stats.rarity, type, b, s);
						} else if (s == SpawnLocation.AirPersistent) {
							storeSpawnInfo(airSpawns, name, stats.rarity, type, b, s);
						} else if (s == SpawnLocation.UnderGround) {
							storeSpawnInfo(undergroundSpawns, name, stats.rarity, type, b, s);
						} else if (s == SpawnLocation.Water || s == SpawnLocation.OnWater) {
							storeSpawnInfo(biomeWaterSpawns, name, stats.rarity, type, b, s);
						}
					}
				}
			}
	...
	public static void getGenerationInfo(HashMap<String, String> hashmap, String name) {
		...
		if (!hashmap.containsKey(name)) {
			if (stats.rarity.day > 0 || stats.rarity.night > 0 || stats.rarity.dawndusk > 0)
				if (ID <= 151) {
					if (PixelmonConfig.Gen1 == true) {
						// System.out.println("Generation 1: " + name);
						SpawnRegistry.addPixelmonSpawn(name, stats, type);
