...
public class ModuleWing extends Module {
	...
	public ModuleWing(ModelRenderer wing, EnumWing WingVariable,
			EnumWing WingOrientation, float WingRotationLimit, float WingSpeed) {
		this.wing = wing;
	...
	public void walk(EntityPixelmon entity, float f, float f1, float f2,
		...
		if (WingOrientation == EnumWing.Horizontal) {
			...
			wing.rotateAngleY = WingDirection * MathHelper.cos(f2 * WingSpeed)
					* (float) Math.PI * WingRotationLimit;
		} else
			wing.rotateAngleZ = WingDirection * MathHelper.cos(f2 * WingSpeed)
					* (float) Math.PI * WingRotationLimit;
