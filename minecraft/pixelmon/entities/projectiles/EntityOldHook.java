				...
				if (RandomHelper.getRandomNumberBetween(1, 10) * 10 <= 90) {
					...
					try {
						EntityPixelmon player1firstPokemon = PixelmonStorage.PokeballManager.getPlayerStorage((EntityPlayerMP) angler).getFirstAblePokemon(
								angler.worldObj);
						bc1 = new BattleController(new PlayerParticipant((EntityPlayerMP) angler, player1firstPokemon), wildpixelmon);
						wildpixelmon.StartBattle(bc1, part);
					} catch (Exception e) {

					}
					this.worldObj.spawnEntityInWorld(magikarp);
					this.angler.addStat(StatList.fishCaughtStat, 1);
					this.angler.sendChatToPlayer("A Pokemon is on the hook!");
				} else {
